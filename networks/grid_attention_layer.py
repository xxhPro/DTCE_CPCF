...
class _GridAttentionBlockND(nn.Module):
    def __init__(
        ...
        # For bilinear/trilinear we should explicitly set align_corners
        # to keep behavior stable across PyTorch versions.
        self.align_corners = False if self.upsample_mode in ["bilinear", "trilinear"] else None
    ...
    def _interp(self, x, size):
        # wrapper to keep calls concise and consistent
        if self.align_corners is None:
            return F.interpolate(x, size=size, mode=self.upsample_mode)
        return F.interpolate(x, size=size, mode=self.upsample_mode, align_corners=self.align_corners)
    ...
    def _concatenation(self, x, g):
        ...
        # g -> phi_g resized to theta_x spatial size
        phi_g = self._interp(self.phi(g), size=theta_x_size[2:])
        f = F.relu(theta_x + phi_g, inplace=True)

        # attention logits -> sigmoid attention map
        sigm_psi_f = torch.sigmoid(self.psi(f))

        # upsample attention map back to input spatial size
        sigm_psi_f = self._interp(sigm_psi_f, size=input_size[2:])
    ...
    def _concatenation_residual(self, x, g):
        ...
        # softmax attention (residual variant)
...
class GridAttentionBlock2D(_GridAttentionBlockND):
    def __init__(self, in_channels, gating_channels, inter_channels=None, mode="concatenation", sub_sample_factor=(2, 2, 2)):
...
class GridAttentionBlock3D_TORR(_GridAttentionBlockND_TORR):
    def __init__(self, in_channels, gating_channels, inter_channels=None, mode="concatenation", sub_sample_factor=(1, 1, 1), bn_layer=True):
        super(GridAttentionBlock3D_TORR, self).__init__(
            in_channels,
            inter_channels=inter_channels,
            gating_channels=gating_channels,
            dimension=3,
