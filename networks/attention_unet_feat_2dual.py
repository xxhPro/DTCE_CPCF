...
class Attention_UNet_feat_2dual(nn.Module):
    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3,
        ...
        super(Attention_UNet_feat_2dual, self).__init__()

        self.params = {'feature_scale': feature_scale,
                       'n_classes': n_classes,
                       'is_deconv': is_deconv,
        ...
        self.encoder = Attention_UNet_Encoder(self.params)
        self.main_decoder = Attention_UNet_Decoder(self.params)
        self.aux_decoder1 = Attention_UNet_Decoder(self.params)

        # 存储特征的属性
        self.last_main_rep = None
        self.last_aux_rep = None
    ...
    def forward(self, x):
        ...
        # 主解码器
        main_output = self.main_decoder(feature)

        # 辅助解码器（应用dropout）
        aux1_feature = [Dropout(i) for i in feature]
        aux_output = self.aux_decoder1(aux1_feature)

        # 存储特征到属性中
        self.last_main_rep = main_output["rep"]
        self.last_aux_rep = aux_output["rep"]
        self.last_encoder_features = feature

        # 保持原来返回格式
        return main_output["pred"], aux_output["pred"]

    def get_features(self):
        """获取上次前向传播的特征"""
        return {
            'main_rep': self.last_main_rep,
            'aux_rep': self.last_aux_rep,
            'encoder_features': self.last_encoder_features
        }
