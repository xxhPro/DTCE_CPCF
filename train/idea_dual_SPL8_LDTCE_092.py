...
parser.add_argument('--conf_thresh', type=float, default=0.92, help='random seed')
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            with (torch.amp.autocast(device_type='cuda', dtype=torch.float16)):
                ...
                # s = (w_main + w_aux).clamp(min=1e-6)
                # w_main_n = (w_main / s).clamp(0.0, 1.0).to(outputs_soft1.device).type_as(outputs_soft1)
                # w_aux_n = (w_aux / s).clamp(0.0, 1.0).to(outputs_soft1.device).type_as(outputs_soft1)


                # alpha = random.random() + 1e-10
                # soft_pseudo_label = outputs_soft1.detach() * w_main_n.unsqueeze(
                #     1) + outputs_soft2.detach() * w_aux_n.unsqueeze(1)
                ...
                per_pixel_ce2 = ce_loss2(outputs_soft2, soft_pseudo_label)
                loss_dtc1 = (per_pixel_ce1 * mask).sum() / mask.sum().clamp(min=1.0)
                loss_dtc2 = (per_pixel_ce2 * mask).sum() / mask.sum().clamp(min=1.0)
                loss_DTC = (1 - consistency_weight) * (loss_dtc1 + loss_dtc2)
...
if __name__ == "__main__":
    ...
    snapshot_path = "/data/hqn_data/Experiment/idea_newal_dual_SPL_LDTCE/092/model/{}_{}/{}_{}".format(
