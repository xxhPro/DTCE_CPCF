...
parser.add_argument('--data_root_path', type=str,
                    default='/data/hqn_data/Data/WORD/3D',
...
def cal_weights(Ni):
    ALPHA = 1 / 3
    Ni = torch.tensor(Ni, dtype=torch.float32, device=device)
    ni = Ni / (Ni.sum() + 1e-6)
    weight = (ni.max() / (ni + 1e-6)) ** ALPHA
    return weight
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            with torch.amp.autocast(device_type='cuda', dtype=torch.float16):
                ...
                pseudo_label = soft_pseudo_label.argmax(dim=1)

                # 统计并构造 weight
                cls_ids, cnts = np.unique(pseudo_label.cpu().numpy(), return_counts=True)
                w_k_cpu = cal_weights(cnts).cpu()  # 在 CPU
                full_w_cpu = torch.ones(num_classes).cpu()  # 在 CPU
                full_w_cpu[cls_ids] = w_k_cpu
                full_w = full_w_cpu.to(device)  # 一次性搬到 cuda

                # 带权 CE on logits + hard labels
                weight_loss = nn.CrossEntropyLoss(weight=full_w,
                                                  ignore_index=num_classes)
                # loss_pse_sup_soft = 0.5 * (ce_loss2(outputs_soft1, soft_pseudo_label) +
                #                            ce_loss2(outputs_soft2, soft_pseudo_label))

                loss_pse_sup_soft = 0.5 * (weight_loss(outputs, pseudo_label) +
                                     weight_loss(outputs_aux1, pseudo_label))
            ...
            if iter_num <= 1000:
                ...
                if iter_num % 200 == 0:
                    logging.info(
                        'iteration %d : loss : %f, loss_ce: %f, loss_wps:%f, alpha: %f' %
...
if __name__ == "__main__":
    ...
    snapshot_path = "/data/hqn_data/WORD_weight3_16/20000/model/{}_{}/{}_{}_{}".format(
